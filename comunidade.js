...
/* checa se o e-mail tem uma assinatura GMP Plus ativa (mesmo critério usado em minha-conta.html) */
...
function seloHtml(tipo, size) {
  if (!tipo) return "";
  size = size || 14;
  return `<span class="selo-verificado ${tipo}" title="${tipo === 'dourado' ? 'GMP Verified+' : 'Assinante GMP Plus'}">
...
function avatarHtml(fotoUrl, size) {
  ...
  return `<div class="com-avatar" style="width:${size}px;height:${size}px">
    <svg viewBox="0 0 24 24" width="${size * 0.48}" height="${size * 0.48}" fill="#ffb366"><path d="M12 12c2.7 0 4.8-2.1 4.8-4.8S14.7 2.4 12 2.4 7.2 4.5 7.2 7.2 9.3 12 12 12zm0 2.4c-3.2 0-9.6 1.6-9.6 4.8v2.4h19.2v-2.4c0-3.2-6.4-4.8-9.6-4.8z"/></svg>
  </div>`;
}
...
function estadoVazio() {
  return `<div class="com-empty">
    <svg width="56" height="56" viewBox="0 0 24 24" fill="#ffb366"><path d="M4 4h16v2H4V4zm0 7h16v2H4v-2zm0 7h16v2H4v-2z"/></svg>
  ...
  <div class="com-post" id="post-${p.id}">
    ...
    <div class="com-post-actions">
      ...
      <button class="com-action-btn ${jaSalvou ? "saved" : ""}" id="btnSave-${p.id}" onclick="salvarPost(${p.id})" style="margin-left:auto">
        <svg width="22" height="22" viewBox="0 0 24 24" fill="${jaSalvou ? "#ffb366" : "none"}" stroke="currentColor" stroke-width="2"><path d="M6 4h12a1 1 0 0 1 1 1v15l-7-4-7 4V5a1 1 0 0 1 1-1z"/></svg>
      </button>
      ${podeExcluir ? `
...
async function salvarPost(postId) {
  ...
  if (jaSalvou) {
    ...
    btn.querySelector("svg").setAttribute("fill", "#ffb366");
...
async function compartilharPost(postId) {
  ...
  if (navigator.share) {
    navigator.share({ title: "Comunidade Equipe GMP", url }).catch(() => {});
...
async function carregarConversasDm() {
  ...
  if (error || !conversas || !conversas.length) {
    lista.innerHTML = `<div class="com-empty">
      <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="#ffb366" stroke-width="1.5"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>
...
async function carregarAtividade(tipo) {
  ...
  lista.innerHTML = data.map(item => {
    ...
    const thumb = post.imagem_url
      ...
      : `<div class="atividade-item-thumb texto">${seloHtml(null)}<svg width="20" height="20" viewBox="0 0 24 24" fill="#ffb366"><path d="M4 4h16v2H4V4zm0 7h16v2H4v-2zm0 7h16v2H4v-2z"/></svg></div>`;
...
/* ── Solicitar / Comprar GMP Verified+ ── */
...
/* compra do GMP Verified+ por R$5 — checkout dedicado (mesmo padrão visual do checkout.html) */
